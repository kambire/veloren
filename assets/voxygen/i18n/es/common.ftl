common-singleplayer = Un jugador
common-multiplayer = Multijugador
common-servers = Servidores
common-quit = Salir
common-settings = Opciones
common-languages = Idiomas
common-interface = Interfaz
common-gameplay = Juego
common-controls = Controles
common-video = Gráficos
common-sound = Sonido
common-chat = Chat
common-networking = Red
common-resume = Reanudar
common-characters = Personajes
common-close = Cerrar
common-yes = Sí
common-no = No
common-back = Atrás
common-create = Crear
common-okay = Vale
common-add = Añadir
common-accept = Aceptar
common-decline = Rechazar
common-disclaimer = Atención
common-cancel = Cancelar
common-none = Ninguno
common-error = Error
common-automatic = Auto
common-empty = Vacío
common-confirm = Confirmar
common-delete_server = Eliminar servidor
common-interface_settings = Ajustes de interfaz
common-gameplay_settings = Ajustes de juego
common-controls_settings = Ajustes de controles
common-video_settings = Ajustes gráficos
common-sound_settings = Ajustes de sonido
common-language_settings = Ajustes de idioma
common-chat_settings = Ajustes de chat
common-networking_settings = Ajustes de red
common-connection_lost =
    ¡Conexión perdida!
    ¿Se reinició el servidor?
    ¿Está el cliente actualizado?
common-species-orc = Orco
common-species-human = Humano
common-species-dwarf = Enano
common-species-elf = Elfo
common-species-draugr = No-muerto
common-species-danari = Danari
common-weapons-axe = Bárbaro (Hacha - DPS Pesado)
common-weapons-dagger = Pícaro (Daga - Sigilo y Críticos)
common-weapons-greatsword = Guerrero (Espadón - Tanque / DPS)
common-weapons-shortswords = Pícaro (Espadas Dobles - DPS Ágil)
common-weapons-sword = Guerrero (Espada y Escudo - Tanque)
common-weapons-staff = Mago (Bastón Ígneo - DPS Mágico)
common-weapons-bow = Cazador (Arco - DPS Distancia)
common-weapons-hammer = Paladín (Martillo Sagrado - Tanque / Soporte)
common-weapons-general = Combate general
common-weapons-sceptre = Sacerdote (Cetro de Sanación - Sanador)
common-weapons-shield = Escudo de Protección
common-weapons-spear = Lancero
common-tool-debug = Depuración
common-tool-farming = Herramienta agrícola
common-tool-pick = Pico
common-tool-mining = Minería
common-tool-instrument = Instrumento
common-kind-modular_component = Componente modular
    .sword = Componente de Espada
    .axe = Componente de Hacha
    .hammer = Componente de Martillo
    .bow = Componente de Arco
    .staff = Componente de Bastón de Fuego
    .sceptre = Componente de Cetro
common-kind-glider = Paravela
common-kind-consumable = Consumible
common-kind-utility = Utilidad
common-kind-ingredient = Ingrediente
common-hands-one = Una mano
common-hands-two = Dos manos
common-rand_appearance = Apariencia aleatoria
common-rand_name = Nombre aleatorio
common-stats-power = Poder
common-stats-speed = Velocidad
common-stats-effect-power = Potencia de estados alterados
common-stats-range = Rango
common-stats-energy_efficiency = Eficiencia de energía
common-stats-buff_strength = Potencia de estados beneficiosos
common-stats-armor = Armadura
common-stats-poise_res = Resistencia al aturdimiento
common-stats-energy_max = Energía máxima
common-stats-energy_reward = Ganancia de energía
common-stats-precision_power = Daño crítico
common-stats-stealth = Sigilo
common-stats-slots = Casillas
common-stats-durability = Durabilidad
common-material-metal = Metal
common-material-wood = Madera
common-material-stone = Piedra
common-material-cloth = Tela
common-material-hide = Piel
common-sprite-chest = Cofre
common-tool-shovel = Pala
common-sprite-crate = Caja
common-accessibility_settings = Opciones de accesibilidad
common-accessibility = Accesibilidad
common-sprite-mud = Barro
common-sprite-grave = Tumba
common-tool-throwable = Lanzable
common-kind-lantern = Linterna
hud-diary-sections-skill_trees-title = Árboles de Habilidades
hud-diary-sections-abilities-title = Habilidades
hud-diary-sections-character-title = Caracter
hud-diary-sections-recipes-title = Recetas
hud-battle-mode = Modo de Batalla
hud-waypoint = Punto de Referencia
common-kind-recipegroup = Recetas
common-kind-quest = Misión
